...
app.post(URL, async (req, res) => { // console.log(req.body);
    ...
    try {
        ...
        if (commands[text]) {
            ...
            const prompt = req.body.message.reply_to_message ?.text + " " + text;
            // Add the previous message's text to the prompt for OpenAI API
            ...
            if (prompt_length > max_tokens) {
                ...
                const response = await openai.complete({engine: 'davinci', prompt: prompt_truncated, max_tokens: max_tokens});
                ...
                const response = await openai.complete({engine: 'davinci', prompt: prompt, max_tokens: max_tokens});
